...
**Version:** 2.0 (managerial analytics, forecasts, and optional AI baseline)
...
## Historical snapshots and completion forecasts

- A snapshot is written only after a successful project sync and only for successfully upserted, non-completed projects. Missing physical progress is stored as `NULL`; an explicit reported numeric `0` remains valid evidence.
- The `(project, Manila capture date)` uniqueness rule stores one daily point. A later successful sync on the same day replaces the earlier point atomically; replaying the same timestamp is idempotent.
- Program, region, province, funding year, and project type are captured with each point so historical dashboard filters use as-captured membership while authorization continues to follow the manager's current server-derived scope.
- Daily history is retained for **365 Manila calendar days**; older snapshots are pruned after successful capture. At the current 25,901-project portfolio size, the conservative full-portfolio ceiling is 9,453,865 rows before completed-project exclusions.
- A completion forecast requires at least three valid physical-progress observations spanning at least 14 days.
- The projected date uses transparent linear-regression velocity. Confidence is derived from sample count, history span, and fit (`R²`); it is not generated by an AI model.
- Nonpositive velocity is labeled **Stalled** without inventing a date. Fewer observations or a shorter span is labeled **Insufficient history**.
- Forecast risk means the evidence-based projected date is later than the reported target date. It does not replace contractual schedule-health rules.
- Forecasts are returned only for the bounded priority-project drill-down, not as an unbounded portfolio transfer.

## Query and optional AI contract

- Portfolio totals, grouped charts, filter options, insights inputs, and rankings are aggregated in PostgreSQL with the same authorization predicates. Only bounded priority and variance rows leave the database.
- The Managerial AI Copilot is disabled unless `ENABLE_MANAGERIAL_AI=true`, requires `analytics:view`, and never calls a provider merely because the dashboard loaded.
- AI tools can read only trusted scoped dashboard DTOs and deterministic risk/forecast evidence. They cannot execute arbitrary SQL, access accounts or citizen PII, mutate projects, infer expenditure, or override official KPI definitions.
- AI commentary is advisory and must preserve the dashboard data timestamp, active filters, canonical project links, and server-derived moderator scope.
- Server-owned AI history is partitioned by an opaque fingerprint of the authenticated user, current authorization scope, and normalized dashboard filters. Filter or assignment changes cannot feed broader-scope prior answers into a narrower conversation.
- PostgreSQL currency aggregates remain in integer-cent form until a guarded bounded conversion for presentation; totals that exceed JavaScript's safe cent range fail instead of silently losing precision.
