...
test("accepts only origins configured for Next image previews", () => {
  assert.equal(isTrustedImagePreviewUrl("https://storage.bafe.online/infra-watch/a.jpg"), true);
...
test("rejects invalid, downgraded, and unapproved historical origins", () => {
  ...
  assert.equal(isTrustedImagePreviewUrl("http://storage.bafe.online/a.jpg"), false);
